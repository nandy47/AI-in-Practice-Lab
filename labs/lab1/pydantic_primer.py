...
class Ticket5(BaseModel):
    ...
    urgency: int = Field(
        ge=1,
        le=5,
        description="Urgency of the ticket on a 1-to-5 scale: 1 means not urgent, while 5 means extremely urgent; 1 = can be answered from general product knowledge or self-service without opening the customer's record; 2 = Aurora must look up the customer's account, take an action, fix a defect, or handle a transaction in flight; 3 = something has already gone wrong or is stuck and the customer is waiting; 4 = repeated failure to resolve, money or access is at risk now, or the customer threatens escalation; 5 = an emergency is in progress, a formal denial demands immediate reversal, or the customer states that they are escalating to the Ombudsman. For the 1/2 boundary, ask whether the issue can be answered without opening the customer's record. For the 4/5 boundary, threatening to escalate is 4, while stating that the customer is already escalating to the Ombudsman is 5. Judge the situation, not the volume: shouting or anger is a sentiment signal, not an urgency signal."
    )
...
def failing_fields(payload: dict) -> list[str]:
    ...
    try:
        ...
        return sorted(error["loc"][0] for error in e.errors())
